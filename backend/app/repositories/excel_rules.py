...
class ExcelRuleRepositoryMixin:
    ...
    def save_excel_rule_draft(self, snapshot: dict[str, Any]) -> dict[str, Any]:
        ...
        with self.connect() as connection:
            ...
            if row:
                connection.execute("UPDATE excel_rule_versions SET snapshot=%s,validation_report='{}',updated_at=%s WHERE id=%s",
                                   (json.dumps(snapshot, ensure_ascii=False), timestamp, row["id"]))
                version_id = row["id"]
            else:
                cursor = connection.execute(
                    "INSERT INTO excel_rule_versions(status,snapshot,validation_report,created_at,updated_at) VALUES('DRAFT',%s,'{}',%s,%s)",
                    (json.dumps(snapshot, ensure_ascii=False), timestamp, timestamp))
                version_id = cursor.lastrowid
            saved = connection.execute("SELECT * FROM excel_rule_versions WHERE id=%s", (version_id,)).fetchone()
        return self._excel_version(saved)

    def record_excel_rule_validation(self, version_id: int, report: dict[str, Any]) -> None:
        with self.connect() as connection:
            connection.execute("UPDATE excel_rule_versions SET validation_report=%s,updated_at=%s WHERE id=%s AND status='DRAFT'",
                               (json.dumps(report, ensure_ascii=False), now_iso(), version_id))

    def publish_excel_rule_draft(self, version_id: int) -> dict[str, Any]:
        timestamp = now_iso()
        with self.connect() as connection:
            row = connection.execute("SELECT * FROM excel_rule_versions WHERE id=%s AND status='DRAFT'", (version_id,)).fetchone()
            if not row:
            ...
            connection.execute("UPDATE excel_rule_versions SET status='PUBLISHED',published_at=%s,updated_at=%s WHERE id=%s",
