...
def test_refresh_replaces_document_only_after_valid_conversion(tmp_path: Path) -> None:
    ...
    refreshed_xml = DOCUMENT_XML.replace(b"report", b"libreoffice changed the control")
    refreshed_xml = refreshed_xml.replace(b">999<", b">4<")
    ...
    assert b">libreoffice changed the control<" not in merged
...
def test_refresh_merges_toc_without_replacing_mapped_controls(tmp_path: Path) -> None:
    document = tmp_path / "report.docx"
    ...
    rendered = '''<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
      <w:body>
        <w:p><w:r><w:t>libreoffice flattened this control</w:t></w:r></w:p>
        <w:sdt><w:sdtPr/><w:sdtContent><w:p>
          <w:r><w:fldChar w:fldCharType="begin"/></w:r>
          <w:r><w:instrText> TOC \\o "1-2" </w:instrText></w:r>
          <w:r><w:fldChar w:fldCharType="separate"/></w:r>
          <w:hyperlink w:anchor="__RefHeading1">
            <w:r><w:t>1.目的</w:t><w:tab/><w:t>4</w:t></w:r>
          </w:hyperlink>
          <w:r><w:fldChar w:fldCharType="end"/></w:r>
        </w:p></w:sdtContent></w:sdt>
        <w:p><w:bookmarkStart w:id="2" w:name="__RefHeading1"/>
          <w:bookmarkStart w:id="1" w:name="_Toc1"/><w:r><w:t>目的</w:t></w:r>
    ...
    assert b'w:tag w:val="mapped-field"' in merged
    assert b">report<" in merged
    assert b'w:line="360"' in merged
    assert merged.count(b'w:sz w:val="22"') == 2
    assert b">4<" in merged
    assert b">999<" not in merged
    assert b"libreoffice flattened this control" not in merged
